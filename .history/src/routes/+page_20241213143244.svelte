<script>
	import { onMount } from 'svelte';
	import { fetchDate, dataStore } from '../Store/store';
	import Carousel from 'svelte-carousel';
	import { paginate, LightPaginationNav } from 'svelte-paginate';

	let loading = true;
	let error = null;
	let currentPage = 1;
	let pageSize = 4;

	// Reactive statement for pagination
	$: paginatedItems = paginate({ items: $dataStore, pageSize, currentPage });

	// Fetch data on mount
	onMount(async () => {
		try {
			await fetchDate(); // Fetch the data and populate the store
		} catch (err) {
			error = 'Failed to fetch data'; // Handle fetch error
		} finally {
			loading = false; // Set loading to false after fetch
		}
	});
</script>

{#if loading}
	<div class="flex h-screen w-full items-center justify-center">loading...</div>
{:else if error}
	<div class="flex h-screen w-full items-center justify-center text-red-500">{error}</div>
{:else if !$dataStore?.length}
	<div class="flex h-screen w-full items-center justify-center">No data available.</div>
{:else}
	<div class="h-screen w-full">
		<h1>Home</h1>
		<Carousel>
			{#each $dataStore as item}
				<div>{item.title}</div>
			{/each}
		</Carousel>
		<ul class="items">
			{#each paginatedItems as item}
				<li class="item">{item.title}</li>
			{/each}
		</ul>

		<LightPaginationNav
			totalItems={$dataStore?.length}
			{pageSize}
			{currentPage}
			limit={1}
			showStepOptions={true}
			on:setPage={(e) => (currentPage = e.detail.page)}
		/>
	</div>
{/if}
